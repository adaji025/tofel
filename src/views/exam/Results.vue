...
export default {
  ...
  methods: {
    ...
    */
    initResult(){
      this.result.id = this.$route.params.result_id;
      this.getResult();
    },
    handleTabs(data) {
    ...
    */
    async getResult()
    {
      this.loader.exam = true;

      const http = this.AxiosHeaders(import.meta.env.VITE_VTALK_API_ENV,'',true);
      ...
      http.get('results/show/'+param).then((response)=>{
          ...
          if(res.success==true)
          {
            ...
            {
              this.result.record = res.data.data.record;
              this.UpdateContent(this.result.record);
            }
          }
...
<template lang="">
  <div id="vt-result-holder">
    <div id="loader-holder" v-if="loader.active">
      <Loader :message="loader.message" :progressBar="parseFloat(progress.bar)"></Loader>
    </div>
        ...
        <div class="flex flex-col sm:flex-row items-center sm:items-start sm:mt-10 gap-10 z-[1]">
          <div class="sm:sticky pt-7 sm:top-[100px] sm:w-1/2">
            <div class="flex flex-col gap-5 items-center justify-center">
              <div class="max-w-[240px] flex flex-col justify-center pb-8 px-10 bg-primary rounded-2xl text-white text-center">
                <div class="text-[96px] leading-tight font-bold">7.5</div>
                <div class="text-lg font-bold mt-5">Estimated Band</div>
              </div>
              ...
              <div class="font-semibold">
                <span class="font-bold">Grammar</span>: 6.5
              </div>
              <div class="font-semibold">
                <span class="font-bold">Vocabulary</span>: 6.5
              </div>
              <div class="font-semibold">
                <span class="font-bold">Fluency & Coherence</span>: 6.5
              </div>
              <div class="font-semibold">
                <span class="font-bold">Pronunciation</span>: 6.5
              </div>
              <div class="font-semibold">
                <span class="font-bold">Speed </span>: 125.8 wpm
              </div>
              <div class="mt-5">
                <Button>Try other exams</Button>
              </div>
            </div>
          </div>
          <div class="flex-1">
